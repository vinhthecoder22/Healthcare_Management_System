package com.vinhthe.patientservice.service.implementation;
...
import com.vinhthe.patientservice.entity.PatientEntity;
import com.vinhthe.patientservice.exception.CustomException;
import com.vinhthe.patientservice.repository.HealthRecordRepository;
import com.vinhthe.patientservice.repository.PatientRepository;
import com.vinhthe.patientservice.service.HealthRecordService;
...
public class HealthRecordServiceImplementation implements HealthRecordService {
    ...
    public HealthRecordDto getHealthRecord(Long healthRecordId) throws CustomException {
        ...
        if (healthRecordEntity.isEmpty() || !healthRecordEntity.get().getIsActive()) {
            throw new CustomException(new ResponseMessageDto("Health record not found", HttpStatus.NOT_FOUND),
                    HttpStatus.NOT_FOUND);
        }
    ...
    public List<HealthRecordDto> getAllHealthRecords() throws CustomException {
        ...
        if (healthRecordEntities.isEmpty()) {
            throw new CustomException(new ResponseMessageDto("No health records found", HttpStatus.NOT_FOUND),
                    HttpStatus.NOT_FOUND);
        }
    ...
    public List<HealthRecordDto> getPatientsHealthRecords(String patientId) throws CustomException {
        ...
        List<HealthRecordEntity> healthRecordEntities = healthRecordRepository
                .findByPatientId(patientEntity.get().getPatientId());
        if (healthRecordEntities.isEmpty()) {
            throw new CustomException(new ResponseMessageDto("No health records found", HttpStatus.NOT_FOUND),
                    HttpStatus.NOT_FOUND);
        }
