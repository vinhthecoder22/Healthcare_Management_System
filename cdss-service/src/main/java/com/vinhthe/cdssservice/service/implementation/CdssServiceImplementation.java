...
import com.vinhthe.cdssservice.constants.OpenAiConstants;
...
import com.vinhthe.cdssservice.repository.HealthRecommendationRepository;
import com.vinhthe.cdssservice.service.CdssService;
...
public class CdssServiceImplementation implements CdssService {
    ...
    private String getResponseFromOpenAi(String prompt)
        ...
        try {
            ...
            OpenAiResponseDto response = restTemplate.postForObject(OpenAiConstants.API_URL, request,
    ...
    private String getResopnseOffline(List<HealthRecordDto> healthRecordDtoList) {
        // Todo: create custom offline recommendation method for generating
        // recommendation
        return "";
    }
    ...
    private String createHealthPrompt(List<HealthRecordDto> healthRecords) {
        ...
        promptBuilder.append(
                "Based on the above information, provide comprehensive health advice or recommendations. The recommendation must be within 2 lines.");
    ...
    public String generateHealthRecommendation()
        ...
        List<HealthRecordDto> healthRecords = patientServiceFeignClient
                .getPatientsHealthRecords(patientDto.getPatientId()).getBody();
        if (healthRecords == null || healthRecords.isEmpty()) {
            throw new CustomException(new ResponseMessageDto("No health records found for the patient.",
        ...
        try {
            ...
            log.error("error in generateHealthRecommendation method from CdssServiceImplementation class: {}",
                    e.getMessage());
            throw new CustomException(
                    new ResponseMessageDto("The cdss service is not available right now. Please try again later.",
                            HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    public List<HealthRecommendationDto> getAllHealthRecommendations() throws CustomException {
        try {
            ...
            if (healthRecommendationList.isEmpty()) {
                throw new CustomException(
                        new ResponseMessageDto("No health recommendations found.", HttpStatus.INTERNAL_SERVER_ERROR),
                        HttpStatus.INTERNAL_SERVER_ERROR);
            }
            ...
            log.error("error in getAllHealthRecommendations method from CdssServiceImplementation class: {}",
                    e.getMessage());
            throw new CustomException(
                    new ResponseMessageDto("The cdss service is not available right now. Please try again later.",
                            HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    public HealthRecommendationDto getHealthRecommendationById(Long id) throws CustomException {
        try {
            ...
            Optional<HealthRecommendationEntity> healthRecommendationEntity = healthRecommendationRepository
                    .findById(id);
            if (healthRecommendationEntity.isEmpty()) {
                throw new CustomException(new ResponseMessageDto("No health recommendation found with the given id.",
                        HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
            }
            ...
            log.error("error in getHealthRecommendationById method from CdssServiceImplementation class: {}",
                    e.getMessage());
            throw new CustomException(
                    new ResponseMessageDto("The cdss service is not available right now. Please try again later.",
                            HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    public List<HealthRecommendationDto> getHealthRecommendationByPatientId(String patientId) throws CustomException {
        try {
            ...
            List<HealthRecommendationEntity> healthRecommendationList = healthRecommendationRepository
                    .findAllByPatientId(patientId);
            if (healthRecommendationList.isEmpty()) {
                throw new CustomException(
                        new ResponseMessageDto("No health recommendations found for the given patient id.",
                                HttpStatus.INTERNAL_SERVER_ERROR),
                        HttpStatus.INTERNAL_SERVER_ERROR);
            }
            ...
            log.error("error in getHealthRecommendationByPatientId method from CdssServiceImplementation class: {}",
                    e.getMessage());
            throw new CustomException(
                    new ResponseMessageDto("The cdss service is not available right now. Please try again later.",
                            HttpStatus.INTERNAL_SERVER_ERROR),
