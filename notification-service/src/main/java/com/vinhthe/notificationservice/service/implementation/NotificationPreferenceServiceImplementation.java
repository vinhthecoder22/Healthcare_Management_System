package com.vinhthe.notificationservice.service.implementation;
...
import com.vinhthe.notificationservice.entity.NotificationPreferenceEntity;
...
import com.vinhthe.notificationservice.repository.NotificationPreferenceRepository;
...
public class NotificationPreferenceServiceImplementation implements NotificationPreferenceService {
    ...
    public void createNotificationPreference(NotificationPreferenceDto notificationPreferenceDto)
        ...
        try {
            ...
            NotificationPreferenceEntity notificationPreferenceEntity = modelMapper.map(notificationPreferenceDto,
                    NotificationPreferenceEntity.class);
            notificationPreferenceRepository.save(notificationPreferenceEntity);

            log.info("notification preference entity saved successfully for user id: {}",
                    notificationPreferenceEntity.getUserId());
        } catch (Exception e) {
            log.error("error occurred while saving notification preference entity for user id: {}",
                    notificationPreferenceDto.getUserId());
            throw new CustomException(new ResponseMessageDto("Notification preference could not be saved",
                    HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @Override
    public NotificationPreferenceDto getNotificationPreferenceByPreferenceType(Long userId, String preferenceType)
            throws CustomException {
        try {
            log.info(
                    "inside getNotificationPreferenceByPreferenceType method of NotificationPreferenceServiceImplementation class");
            Optional<NotificationPreferenceEntity> entity = notificationPreferenceRepository
                    .findByUserIdAndPrefType(userId, PreferenceType.valueOf(preferenceType));
            if (entity.isEmpty()) {
                throw new CustomException(
                        new ResponseMessageDto("Notification preference not found", HttpStatus.NOT_FOUND),
                        HttpStatus.NOT_FOUND);
            }
            log.info("notification preference entity fetched successfully for user id: {}", userId);
            return new NotificationPreferenceDto(entity.get().getNotificationPreferenceId(), entity.get().getUserId(),
                    entity.get().getPrefType(), entity.get().isEnabled());
        } catch (Exception e) {
            log.error("error occurred while fetching notification preference entity for user id: {}", userId);
            throw new CustomException(new ResponseMessageDto("Notification preference could not be fetched",
                    HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public List<NotificationPreferenceDto> getAllNotificationPreferencesByUserId(Long userId) throws CustomException {
        try {
            log.info(
                    "inside getAllNotificationPreferencesByUserId method of NotificationPreferenceServiceImplementation class");

            List<NotificationPreferenceEntity> entities = notificationPreferenceRepository.findAllByUserId(userId);
            if (entities.isEmpty()) {
                throw new CustomException(
                        new ResponseMessageDto("Notification preferences not found", HttpStatus.NOT_FOUND),
                        HttpStatus.NOT_FOUND);
            }
            ...
            throw new CustomException(new ResponseMessageDto("Notification preferences could not be fetched",
                    HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public void updateNotificationPreference(NotificationPreferenceDto notificationPreferenceDto)
        ...
        try {
            ...
            NotificationPreferenceEntity updatedEntity = modelMapper.map(notificationPreferenceDto,
                    NotificationPreferenceEntity.class);
            ...
            log.error("error occurred while updating notification preference entity for id: {}",
                    notificationPreferenceDto.getNotificationPreferenceId());
            throw new CustomException(new ResponseMessageDto("Notification preference could not be updated",
                    HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public void deleteNotificationPreference(Long notificationPreferenceId) throws CustomException {
        try {
            ...
            Optional<NotificationPreferenceEntity> entity = notificationPreferenceRepository
                    .findById(notificationPreferenceId);
            if (entity.isEmpty()) {
                throw new CustomException(
                        new ResponseMessageDto("Notification preference not found", HttpStatus.NOT_FOUND),
                        HttpStatus.NOT_FOUND);
            ...
            log.error("error occurred while deleting notification preference entity for id: {}",
                    notificationPreferenceId);
            throw new CustomException(new ResponseMessageDto("Notification preference could not be deleted",
                    HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
