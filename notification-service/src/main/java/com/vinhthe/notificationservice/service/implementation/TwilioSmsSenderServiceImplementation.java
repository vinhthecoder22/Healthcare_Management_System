...
import com.vinhthe.notificationservice.config.TwilioConfiguration;
...
public class TwilioSmsSenderServiceImplementation implements TwilioSmsSenderService {
    ...
    public void sendSms(String phoneNumberTo, String message)
        ...
        try {
            ...
            Message.creator(new PhoneNumber(phoneNumberTo), new PhoneNumber(twilioConfiguration.getFromNumber()),
                    message).create();
            log.info("sms sent successfully to {}", phoneNumberTo);
        } catch (Exception e) {
            log.error("error sending sms to {}", phoneNumberTo);
            throw new CustomException(new ResponseMessageDto("Error sending sms", HttpStatus.BAD_REQUEST),
